...
const handleDelete = (val)=>{
  //proxy.$api
  ElMessageBox.confirm("确认要删除吗？").then(async ()=>{
...
const handleEdit = (val)=>{
  action.value = 'edit'
  dialogVisible.value = true
  nextTick(()=>{    //这个生效很快，所以用一个nextTick防止编辑内容成为新增的默认内容
    Object.assign(formUser,{...val,sex:''+val.sex})  //转换成字符串
  })
}
...
<template>
    ...
    会对el-select的样式造成影响，我们通过给他设置一个class=select-clearn在css进行处理-->
